...
c_min = np.array([0, 125, 125])
c_max = np.array([19, 255, 255])    
...
def index_emax(cnt): 
    ...
    for i in range(len(cnt)):
        #print("object:", i)
        #print("cnt points:",len(cnt[i]))
        cnt_num=len(cnt[i]) # get the total amount of points that made object's contour line
        if cnt_num > max_num:
            max_num = cnt_num
            max_i = i
...
def main():
    ...
    while True:
        ...
        # 色領域の輪郭を抽出 
        # findContours( image, extract_mode, approximate method)
        ...
        n = index_emax(cnt) # cnt is list of objects's contour

        if n != -1:
            #print("object\n", cnt[n])
            # Create new convex shape
            ...
            #cv2.drawContours(im,[hull],-1,(0,200,0),2)
            #cv2.drawContours(mask2,[hull],-1,(0,200,0),2)
            M1 = cv2.moments(cnt[n])
            #print(M1)
            if M1["m00"] != 0:
               cx,cy = int(M1["m10"]/M1["m00"]),int(M1["m01"]/M1["m00"]) # find centroid of object's blob
            else:
                cx,cy = 320,240
        else:
            cx,cy = 320,240
