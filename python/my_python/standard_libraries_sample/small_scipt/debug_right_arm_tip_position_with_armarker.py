...
class Sciurus(object):
    ...
    def callback_get_marker_pos(self, markers):
        for m in markers.markers:
            ...
            self.marker_pose_list.append(pos)
            self.marker_orientation_list.append(ori)

            self.last_seen_marker = rospy.get_rostime().secs

            print "num of marker:", len(self.marker_pose_list)
            if len(self.marker_pose_list) > 10: del(self.marker_pose_list[0])
            if len(self.marker_orientation_list) > 10: del(self.marker_orientation_list[0])
            

    def remove_old_marker(self):
        if rospy.get_rostime().secs - self.last_seen_marker > 0.5 and len(self.marker_pose_list):
            del(self.marker_pose_list[0])

    def get_armarker_avg_pos(self):
        if len(self.marker_pose_list) > 5:
            pos_x = sum([m.x for m in self.marker_pose_list])/len(self.marker_pose_list)
            pos_y = sum([m.y for m in self.marker_pose_list])/len(self.marker_pose_list)
            pos_z = sum([m.z for m in self.marker_pose_list])/len(self.marker_pose_list)
            return (pos_x, pos_y, pos_z)
        return None

    def debug_arm_tip_with_ar_marker(self):
        ar_pose = self.get_armarker_avg_pos()
        arm_pose = arm.get_current_pose().pose
        
        if ar_pose:
            self.output_message("DEBBUG HAND POSITION WITH AR MARKER")
            rospy.loginfo("marker position (x, y, z) = (%3.3f: %3.3f: %3.3f)" %(ar_pose[0], ar_pose[1], ar_pose[2])) 
            #print "Position Different"
            #print "X:", (arm_pose.position.x - ar_pose.x), "Y:", (arm_pose.position.y - ar_pose.y), "Z:", (arm_pose.position.z - ar_pose.z)
            ...
            rospy.logwarn("NO AR MARKER FOUND")
